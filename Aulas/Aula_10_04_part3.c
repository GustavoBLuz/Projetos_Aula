...
// a menor nota do grupo
int main (void)
{
   float nota1, mediaAp,mediaRp;
   float somaNotasAPR = 0, somaNotasRPR = 0;
   int controle;
   int alunosAPR = 0, alunosRPR = 0;
   float menorNota =10, maiorNota = 0;
   ...
   printf("\nA media dos alunos APR eh: %.1f", mediaAp);
   printf("\nO numero de alunos RPR eh: %d", alunosRPR);
   printf("\nA media dos alunos RPR eh: %.1f", mediaRp);
   printf("\nA menor nota do grupo eh: %.1f", menorNota);
   printf("\nA maior nota do grupo eh: %.1f\n", maiorNota);
